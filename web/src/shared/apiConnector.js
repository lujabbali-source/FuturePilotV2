...
/** Trae el resultado guardado en la cuenta (si la hay) y repinta la pagina.
 *
 *  Antes esto lo disparaba un listener de DOMContentLoaded dentro de este
 *  mismo archivo, que miraba window.location.pathname para adivinar en que
 *  pagina estaba. Ahora cada pagina llama explicitamente a lo suyo: el
 *  modulo no tiene por que saber que rutas existen ni como se llaman.
 *
 *  Pinta primero lo que haya en local (inmediato, sin esperar a la red) y
 *  repinta con lo de la cuenta, que es la fuente de verdad y puede traer un
 *  resultado mas reciente hecho en otro dispositivo. */
export async function hydrate(render) {
render();
if (await refreshResultsFromServer()) render();
}
