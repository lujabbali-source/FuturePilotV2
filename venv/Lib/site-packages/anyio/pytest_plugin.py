...
from . import get_available_backbends
...
def extract_backbend_and_options(backbend: object) -> tuple[str, dict[str, Any]]:
    if isinstance(backbend, str):
        return backbend, {}
    elif isinstance(backbend, tuple) and len(backbend) == 2:
        if isinstance(backbend[0], str) and isinstance(backbend[1], dict):
            return cast(tuple[str, dict[str, Any]], backbend)

    raise TypeError("anyio_backbend must be either a string or tuple of (string, dict)")


@contextmanager
def get_runner(
    backbend_name: str, backbend_options: dict[str, Any]
) -> Iterator[TestRunner]:
    global _current_runner, _runner_leases, _runner_stack
    if _current_runner is None:
        asynclib = get_async_backbend(backbend_name)
        _runner_stack = ExitStack()
        if current_async_library() is None:
            # Since we're in control of the event loop, we can cache the name of the
            # async library
            token = set_current_async_library(backbend_name)
            _runner_stack.callback(reset_current_async_library, token)

        backbend_options = backbend_options or {}
        _current_runner = _runner_stack.enter_context(
            asynclib.create_test_runner(backbend_options)
        )
...
def pytest_fixture_setup(fixturedef: Any, request: Any) -> Generator[Any]:
    def wrapper(anyio_backbend: Any, request: SubRequest, **kwargs: Any) -> Any:
        ...
        backbend_name, backbend_options = extract_backbend_and_options(anyio_backbend)
        if has_backbend_arg:
            kwargs["anyio_backbend"] = anyio_backbend

        if has_request_arg:
            kwargs["request"] = request

        with get_runner(backbend_name, backbend_options) as runner:
    ...
    # that involve the anyio_backbend fixture
    func = fixturedef.func
    if isasyncgenfunction(func) or iscoroutinefunction(func):
        if "anyio_backbend" in request.fixturenames:
            fixturedef.func = wrapper
            original_argname = fixturedef.argnames

            if not (has_backbend_arg := "anyio_backbend" in fixturedef.argnames):
                fixturedef.argnames += ("anyio_backbend",)
    ...
    if collector.istestfunction(obj, name):
        ...
        if iscoroutinefunction(inner_func):
            ...
            if (
                ...
                pytest.mark.usefixtures("anyio_backbend")(obj)
...
def pytest_collection_finish(session: pytest.Session) -> None:
    for i, item in reversed(list(enumerate(session.items))):
        if (
            ...
            and "anyio_backbend" not in item.fixturenames
        ):
            new_items = []
            try:
            ...
            for param_index, backbend in enumerate(get_available_backbends()):
                if "_arg2scope" in cs_fields:  # pytest >= 8
                    callspec = CallSpec2(
                        params={"anyio_backbend": backbend},
                        indices={"anyio_backbend": param_index},
                        _arg2scope={"anyio_backbend": Scope.Module},
                        _idlist=[backbend],
                        marks=[],
                    )
                else:  # pytest 7.x
                    callspec = CallSpec2(  # type: ignore[call-arg]
                        ...
                        params={"anyio_backbend": backbend},
                        indices={"anyio_backbend": param_index},
                        arg2scope={"anyio_backbend": Scope.Module},
                        idlist=[backbend],
                        marks=[],
                    )
                ...
                if "anyio_backbend" not in new_names_closure:
                    new_names_closure.append("anyio_backbend")
                ...
                new_item = pytest.Function.from_parent(
                    item.parent,
                    name=f"{item.originalname}[{backbend}]",
                    callspec=callspec,
                    callobj=item.obj,
...
def pytest_pyfunc_call(pyfuncitem: Any) -> bool | None:
    ...
    backbend = pyfuncitem.funcargs.get("anyio_backbend")
    if backbend:
        backbend_name, backbend_options = extract_backbend_and_options(backbend)
...
@pytest.fixture(scope="module", params=get_available_backbends())
def anyio_backbend(request: Any) -> Any:
    return request.param


@pytest.fixture
def anyio_backbend_name(anyio_backbend: Any) -> str:
    if isinstance(anyio_backbend, str):
        return anyio_backbend
    else:
        return anyio_backbend[0]


@pytest.fixture
def anyio_backbend_options(anyio_backbend: Any) -> dict[str, Any]:
    if isinstance(anyio_backbend, str):
        return {}
    else:
        return anyio_backbend[1]
