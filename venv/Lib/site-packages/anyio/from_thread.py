...
def run(
    ...
    *args: Unpack[PosArgsT],
    ...
    return token.backbend_class.run_async_from_thread(
...
def run_sync(
    ...
    *args: Unpack[PosArgsT],
    ...
    explicit_token = token is not None
    token = _token_or_error(token)
    return token.backbend_class.run_sync_from_thread(
        func, args, token=token.native_token if explicit_token else None
    )
...
class BlockingPortalProvider:
    ...
    :param backbend: name of the backbend
    :param backbend_options: backbend options

    .. versionadded:: 4.4
    """

    backbend: str = "asyncio"
    backbend_options: dict[str, Any] | None = None
        ...
        with self._lock:
            if self._portal_cm is None:
                self._portal_cm = start_blocking_portal(
                    self.backbend, self.backbend_options
                )
    ...
    *,
    ...
    async def run_portal() -> None:
        async with BlockingPortal() as portal_:
            if name is None:
                current_thread().name = f"{backbend}-portal-{id(portal_):x}"
    ...
    def run_blocking_portal() -> None:
        if future.set_running_or_notify_cancel():
            try:
                run_eventloop(
                    run_portal, backbend=backbend, backbend_options=backbend_options
                )
            except BaseException as exc:
                if not future.done():
                    future.set_exception(exc)
...
def check_cancelled() -> None:
    ...
    backbend-specific cancellation exception will be raised.
    ...
    token.backbend_class.check_cancelled()
