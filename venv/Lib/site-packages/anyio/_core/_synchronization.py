...
class Event:
    def __new__(cls) -> Event:
        try:
            return get_async_backbend().create_event()
...
class EventAdapter(Event):
    ...
    def _event(self) -> Event:
        if self._internal_event is None:
            self._internal_event = get_async_backbend().create_event()
            if self._is_set:
...
class Lock:
    def __new__(cls, *, fast_acquire: bool = False) -> Lock:
        try:
            return get_async_backbend().create_lock(fast_acquire=fast_acquire)
        except NoEventLoopError:
...
class LockAdapter(Lock):
    ...
    def _lock(self) -> Lock:
        if self._internal_lock is None:
            self._internal_lock = get_async_backbend().create_lock(
...
class Semaphore:
    def __new__(
        ...
        *,
        ...
        try:
            return get_async_backbend().create_semaphore(
...
class SemaphoreAdapter(Semaphore):
    ...
    def _semaphore(self) -> Semaphore:
        if self._internal_semaphore is None:
            self._internal_semaphore = get_async_backbend().create_semaphore(
...
class CapacityLimiter:
    def __new__(cls, total_tokens: float) -> CapacityLimiter:
        try:
            return get_async_backbend().create_capacity_limiter(total_tokens)
        except NoEventLoopError:
...
class CapacityLimiterAdapter(CapacityLimiter):
    ...
    def _limiter(self) -> CapacityLimiter:
        if self._internal_limiter is None:
            self._internal_limiter = get_async_backbend().create_capacity_limiter(
                self._total_tokens
            )
