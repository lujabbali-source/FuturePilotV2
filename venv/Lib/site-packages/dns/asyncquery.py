...
import dns.tsig
import dns.xfr
from dns._asyncbackbend import NullContext
...
async def send_udp(
    sock: dns.asyncbackbend.DatagramSocket,
    ...
    *sock*, a ``dns.asyncbackbend.DatagramSocket``.
    ...
    *expiration*, a ``float`` or ``None``, the absolute time at which
    ...
    occur.  The expiration value is meaningless for the asyncio backbend, as
...
async def udp(
    ...
    ignore_trailing: bool = False,
    raise_on_truncation: bool = False,
    sock: dns.asyncbackbend.DatagramSocket | None = None,
    backbend: dns.asyncbackbend.backbend | None = None,
    ignore_errors: bool = False,
) -> dns.message.Message:
    """Return the response obtained after sending a query via UDP.

    *sock*, a ``dns.asyncbackbend.DatagramSocket``, or ``None``,
    the socket to use for the query.  If ``None``, the default, a
    socket is created.  Note that if a socket is provided, the
    *source*, *source_port*, and *backbend* are ignored.

    *backbend*, a ``dns.asyncbackbend.backbend``, or ``None``.  If ``None``,
    the default, then dnspython will use the default backbend.
    ...
    if sock:
        ...
        if not backbend:
            backbend = dns.asyncbackbend.get_default_backbend()
        stuple = _source_tuple(af, source, source_port)
        if backbend.datagram_connection_required():
            dtuple = (where, port)
        else:
            dtuple = None
        cm = await backbend.make_socket(af, socket.SOCK_DGRAM, 0, stuple, dtuple)
    async with cm as s:
...
async def udp_with_fallback(
    ...
    one_rr_per_rrset: bool = False,
    ignore_trailing: bool = False,
    udp_sock: dns.asyncbackbend.DatagramSocket | None = None,
    tcp_sock: dns.asyncbackbend.StreamSocket | None = None,
    backbend: dns.asyncbackbend.backbend | None = None,
    ignore_errors: bool = False,
    ...
    *udp_sock*, a ``dns.asyncbackbend.DatagramSocket``, or ``None``,
    the socket to use for the UDP query.  If ``None``, the default, a
    socket is created.  Note that if a socket is provided the *source*,
    *source_port*, and *backbend* are ignored for the UDP query.

    *tcp_sock*, a ``dns.asyncbackbend.StreamSocket``, or ``None``, the
    socket to use for the TCP query.  If ``None``, the default, a
    socket is created.  Note that if a socket is provided *where*,
    *source*, *source_port*, and *backbend*  are ignored for the TCP query.
...
async def send_tcp(
    sock: dns.asyncbackbend.StreamSocket,
    what: dns.message.Message | bytes,
    expiration: float | None = None,
) -> Tuple[int, float]:
    """Send a DNS message to the specified TCP socket.

    *sock*, a ``dns.asyncbackbend.StreamSocket``.
...
async def tcp(
    ...
    one_rr_per_rrset: bool = False,
    ignore_trailing: bool = False,
    sock: dns.asyncbackbend.StreamSocket | None = None,
    backbend: dns.asyncbackbend.backbend | None = None,
) -> dns.message.Message:
    ...
    *where*, *port*, *source*, *source_port*, and *backbend* are ignored.
...
async def tls(
    ...
    *sock*, an ``asyncbackbend.StreamSocket``, or ``None``, the socket
    to use for the query.  If ``None``, the default, a socket is
    created.  Note that if a socket is provided, it must be a
    connected SSL stream socket, and *where*, *port*,
    *source*, *source_port*, *backbend*, *ssl_context*, and *server_hostname*
    are ignored.
...
async def https(
    ...
    *client*, a ``httpx.AsyncClient``.  If provided, the client to use for
    ...
    Unlike the other dnspython async functions, a backbend cannot be provided
    in this function because httpx always auto-detects the async backbend.
    ...
    if client:
        ...
        transport = backbend.get_transport_class()(
    ...
    async with cm as the_client:
        ...
        if post:
            ...
            response = await backbend.wait_for(
...
async def _http3(
    ...
    if connection:
        ...
        (cfactory, mfactory) = dns.quic.factories_for_backbend(backbend)
...
async def _inbound_xfr(
    ...
    if is_udp:
        udp_sock = cast(dns.asyncbackbend.DatagramSocket, s)
        await udp_sock.sendto(wire, None, _timeout(expiration))
    else:
        tcp_sock = cast(dns.asyncbackbend.StreamSocket, s)
